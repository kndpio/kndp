...
func handleChartValues(w http.ResponseWriter, r *http.Request) {
	// Extract the chart name from the URL path
	chartName := strings.TrimPrefix(r.URL.Path, "/api/stack/")
...
func main() {
	...
	http.Handle("/api/stack/", corsHandler.Handler(http.HandlerFunc(handleChartValues)))
